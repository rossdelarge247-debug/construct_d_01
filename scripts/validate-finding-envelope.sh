...
printf '%s' "$INPUT" | jq -e '
  .specialist as $s |
  $s | (. == "reviewer-security" or . == "reviewer-architecture" or . == "reviewer-correctness" or . == "reviewer-style")
' >/dev/null \
  || fail "specialist must be one of: reviewer-security, reviewer-architecture, reviewer-correctness, reviewer-style"
