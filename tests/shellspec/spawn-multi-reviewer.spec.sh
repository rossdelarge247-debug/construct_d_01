...
Describe 'spawn-multi-reviewer.sh aggregate'
  ...
  It 'returns block at k=1 with shadow downgrade at k=2/k=3 (AC-1 verification 2)'
    ...
    write_envelope security "$EMPTY_FINDINGS_SECURITY"
    write_envelope architecture '{"specialist":"reviewer-architecture","summary":"hidden state","findings":[{"label":"issue","blocking":true,"category":"hidden-effect","summary":"module-level Date.now()","evidence":"const T = Date.now()","remediation":"inject Clock interface"}]}'
    write_envelope correctness '{"specialist":"reviewer-correctness","summary":"sub","findings":[{"label":"suggestion","blocking":false,"category":"spec-citation","summary":"unquoted spec ref","evidence":"per spec X","remediation":"quote it"}]}'
    write_envelope style '{"specialist":"reviewer-style","summary":"nit","findings":[{"label":"nitpick","blocking":false,"category":"naming","summary":"filler noun","evidence":"data: any","remediation":"rename"}]}'
    When call scripts/spawn-multi-reviewer.sh aggregate "$SHELLSPEC_TMPBASE"
  ...
  It 'dedupes cross-specialist findings via SHA-256-equivalent hash and counts seen_by length as votes (AC-1 verification 5)'
    ...
    # votes toward the verdict tier. With 2 specialists flagging the same
    # blocking finding, at k=2 BLOCKING_VOTES=2 ≥ 2 → block stays. At
    # k=3, 2 < 3 → falls to approve (action tier excludes blocking).
    write_envelope security '{"specialist":"reviewer-security","summary":"x","findings":[{"label":"issue","blocking":true,"category":"security","summary":"unsanitised input boundary","evidence":"req.query.id","remediation":"validate at boundary"}]}'
    write_envelope correctness '{"specialist":"reviewer-correctness","summary":"x","findings":[{"label":"issue","blocking":true,"category":"security","summary":"unsanitised input boundary","evidence":"req.query.id","remediation":"validate at boundary"}]}'
    write_envelope architecture "$EMPTY_FINDINGS_ARCH"
    write_envelope style "$EMPTY_FINDINGS_STYLE"
    When call scripts/spawn-multi-reviewer.sh aggregate "$SHELLSPEC_TMPBASE"
    The output should include '"verdict": "block"'
    ...
    The output should include '"would_have_been_k3": "approve"'
    The status should be success
    # finding count should be 1 after dedupe (not 2)
    The output should include '"summary": "unsanitised input boundary"'
    The output should not include '"correctness",\n        "correctness"'
  End
