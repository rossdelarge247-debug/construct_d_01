...
- **Investigation surfaced the canvas-coverage gap before refactor began.** The user asked "are the canvases enough to refactor against?" — the decoded sibling for o7 was 922 lines (suspiciously close to o1's 1295 but missing layout depth). `grep -c "<div"` on the two showed 0 vs 200+. Catching this BEFORE writing src/ saved a refactor based on guess-work.

> **Diagnostic correction (session 79):** The session-78 root-cause attribution — *"o7 v1 used external `<script type="text/babel" src="…sandbox-bundler-canvas/<uuid>.js">` references that don't survive export"* — was wrong. The o7 v1 canvas IS self-contained. The actual bug was in `scripts/decode-bundler-canvas.sh`: the script extracted the `<script type="__bundler/template">` JSON-string but did not apply the manifest-substitution step the in-browser loader performs (extract `__bundler/manifest`, base64-decode each entry, gunzip when `compressed: true`, replace UUID placeholders in the template with `data:` URLs for assets and inline content for `text/babel` script-src refs). Session 79 fixes the decoder; the `o7-canvas-prompt.md` v2 (authored under the wrong diagnosis) is removed; o7 raw canvas needs no re-export.
