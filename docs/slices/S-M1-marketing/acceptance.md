...
**AC-2 · Hero variant set (in-scope = 1 of 9; remainder → S-M1.0a).** This slice ships **HeroEditorial** as the production default and the forward-extensible map shape: `src/components/marketing/heroes/index.ts` exports `HeroEditorial` as a named export, `SELECTED_HERO_VARIANT` (default value = `'editorial'`), and a `HERO_VARIANTS` map keyed by the kebab-case variant slug (1 key at S-M1 ship; 9 keys post-S-M1.0a). HeroEditorial renders without throwing under jsdom and contains its variant-distinguishing visual element — the central document spine + 4 §-numbered area cards + 4 orbiting cards labeled Area 01-04. The remaining 8 variants (`HeroDeclarative`, `HeroTypographic`, `HeroProductForward`, `HeroOutcomeLed`, `HeroTwoColumn`, `HeroEmpathetic`, `HeroAtmospheric`, `HeroDiagrammatic`) and their per-variant smoke tests are owned by **S-M1.0a follow-up slice**; the `HERO_VARIANTS` map shape is forward-extensible so additional keys land without breaking consumers.

*Evidence:* `src/components/marketing/heroes/index.test.ts` — exports contract (named export `HeroEditorial` + `SELECTED_HERO_VARIANT='editorial'` + `HERO_VARIANTS` map with `'editorial'` key); `src/components/marketing/heroes/editorial.test.tsx` — HeroEditorial smoke test asserting the central document spine + §1-§4 area prefixes + 4 orbit-card labels (signature element). The 8 remaining variant exports + per-variant smoke tests defer to S-M1.0a's acceptance contract.
