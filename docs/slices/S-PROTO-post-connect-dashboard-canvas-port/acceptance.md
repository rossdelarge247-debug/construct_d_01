...
**AC-7.** `DisclosureCard` (canvas L1455–1485) renders the "Your private area" disclosure-preview card with a kicker, italic-serif H3 title ("View your disclosure picture"), descriptive body copy, a 32% progress bar, and an INK CTA button ("Go to your picture"). Receives `variant`. Expressive variant uses `#9D174D` for kicker + progress accents; conservative uses `MUTE` + `INK`. *Evidence:* visual matches canvas at both variants.
