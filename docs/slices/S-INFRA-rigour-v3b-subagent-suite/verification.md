...
**Outcome (per acceptance.md L22-26):** every PR that touches `src/` receives an automatic adversarial code review from a fresh-context subagent on `pull_request:opened/synchronize`, surfacing edge cases / security / regression / AC gaps / scope-creep within the diff. Verdict posts as a check run; informational at v3b ship (no merge gate per AC-1 §Out of scope).
...
3. **`.claude/hooks-checksums.txt`** — re-baselined with new entry `88c4481786046919b899ceac80927521c16f0714a49dcecd98b350faa120ef36  .claude/agents/slice-reviewer.md` per AC-1 verification ("integrity-protected via `hooks-checksums.txt` baseline"). Total entries 17 → 20.
...
**Outcome (per acceptance.md L28-32):** at slice-completion (called by `/wrap` or main session), a subagent walks the slice's AC list and renders per-AC pass/fail-with-narrative, distinct from `verify-slice.sh`'s file-presence + tooling-runs check. Catches papered-over evidence, missing Loveable check fields, AC quantity outside §C-template bounds.
...
2. **`.claude/hooks-checksums.txt`** — new entry `fbe9006f981740c276cbdca7e80944660b7140d7befd7f55e542b479760b35bd  .claude/agents/acceptance-gate.md`.
...
**Outcome (per acceptance.md L34-38):** every src/ slice with UI surface receives an 8-criterion polish-floor review (6 dimensions per spec 72a + spec 26 motion + spec 73 copy) at slice-completion. Catches missing `prefers-reduced-motion` fallbacks, keyboard-trap, mobile breakage, missing aria-live regions. **Dormant at v3b ship; active from S-F1 onwards.**
...
2. **`.claude/hooks-checksums.txt`** — new entry `04649df20abd94996c58b8a610c300c6539841faeacce7e11b36a7dbf3bf2601  .claude/agents/ux-polish-reviewer.md`.
