...
- **Verification:** `package.json` devDependency `@vitest/coverage-v8` pinned to a specific minor (TBD by drafter at impl time). `vitest.config.ts` includes `coverage: { reporter: ['lcov'], reportsDirectory: './coverage' }` (or equivalent) producing `coverage/lcov.info`. `.github/workflows/ci.yml` test job runs `pnpm vitest --coverage` and uploads `coverage/` as artefact. Verify-slice.sh Gate 5 reads `coverage/lcov.info` and enforces ≥90% threshold per v3a `acceptance.md` L51 + L178 (v3b inherits v3a's coverage floor; does not regress it). Attribution corrected at v3b S-5 wrap (was "spec 72 F6" pre-fix; spec 72 has no F6/coverage references — F6 is a v3a-internal acceptance label). CI run on this commit shows non-zero coverage data (sanity floor).
