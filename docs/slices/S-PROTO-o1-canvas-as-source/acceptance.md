...
### AC-6 — Cross-screen header + footer chassis unification (in-PR scope expansion)

Authorized in-PR mid-review on observed visual inconsistency across O1 (canvas-as-source · this slice), O2 (canvas-as-source · prior slice), and O3-O8 (rebuild via `ScreenShell`). User direction: "I think the nav bar on 01 is the most accurate" + "the button footer bar is different across screens — I think 01 is the best version". The Out-of-scope note below at L53 ("Stepper component rewrite") is RESOLVED here; "Removing `ScreenShell` from `src/`" remains deferred (still consumed by O3-O8).

- **AC-6.a — Shared `<Arrow />` component.** `src/app/dev/proto/pre-signup-interview/components/Arrow.tsx` (NEW) wraps the canvas-faithful shaft + arrowhead matching decoded canvas L766-771 (`const Arrow = ({size=14, sw=1.8, dir="right"}) => { ... <line x1="5" y1="12" x2="19" y2="12"/> <polyline points="12 5 19 12 12 19"/> ... }`) with 4-direction rotation via CSS `transform`. O1 + O2 stop defining local `Arrow`; both import from shared. O1's CTA Arrow gains `strokeWidth={2}` per canvas L1093 `sw={2}`.
- **AC-6.b — `ProgressPill` as canonical step indicator.** O2's local `StepRail` (functionally identical to `ProgressPill` apart from text casing) deleted; O2's `TopBar` imports + renders `<ProgressPill step={step} total={total} />`. `ProgressPill` label re-skinned: visible text reads `STEP X / Y` via `textTransform: uppercase` + `tokens.font.mono` + `letterSpacing: 0.08em` to match the canvas eyebrow pattern; DOM `textContent` + `aria-label` preserved as "Step X / Y" so screen readers read the word normally and the existing test assertion (`tests/unit/proto-pre-signup/progress-pill.test.tsx` L49 `expect(container.textContent).toContain('Step 3 / 8')`) passes unchanged.
- **AC-6.c — `ScreenShell` footer chassis matches O1.** `border-top: 1px solid ${tokens.color.border}` divider, `background: rgba(245,245,244,0.85)` + `backdropFilter: blur(8px)`, trust band default `{ left: 'Free', right: 'Private until saved' }` above the button (mono uppercase 10.5px MUTED text with middle-dot separator), dark pill button (`14/18px padding, borderRadius: 999, fontSize: 14, fontWeight: 600`) with right-arrow at `strokeWidth=2`. New optional `trustBand?: { left, right }` prop; existing `ctaCaption?` preserved as override (renders single-line if passed; trust band renders if not). `PrimaryCTA` component deleted (no remaining callers).
- **AC-6.d — `ScreenShell` outer wrapper matches O1's per-section padding pattern.** Prior `<main>`-level `padding: '24px 20px 48px'` + `gap: 28` replaced with `<main>` carrying only `paddingTop: 24` + `maxWidth: 480` + flex column, and per-section internal padding (header `16px 20px 12px`, heading `20px 20px 16px`, children `0 20px`, footer `12px 20px 20px`). Footer becomes a full-bleed bar within the 480px frame. Cross-screen visual chassis (BrandBar → header → heading → children → footer) is now consistent in chrome across O1 + O2 + O3-O8; content varies per screen.
