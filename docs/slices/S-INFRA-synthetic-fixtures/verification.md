...
| AC-3 | Live-invocation runner | PASS | `tests/personas/run-synthetic.sh` (~100L) + `tests/personas/match-synthetic.sh` (~60L). Both `bash -n` clean. Matcher covered by `tests/shellspec/match-synthetic.spec.sh` — 9 cases: PASS-all-predicates + 6 per-predicate FAIL paths (count / label / blocking / category / evidence / remediation) + 2 missing-input-file precondition exit-2 cases. Runner skip-on-no-API-key tested: unset `ANTHROPIC_API_KEY` → exit 0 with `SKIP` diagnostic to stderr. Runner CLI-version semver guard: invalid value (e.g. non-semver) → exit 2 with diagnostic. |
...
- [x] Matcher covered by `tests/shellspec/match-synthetic.spec.sh` (9 cases:
  1 PASS-all-predicates + 6 per-predicate FAIL + 2 missing-input precondition)
...
| 2 | Tests written + passing | ✅ | Matcher covered by 9-case ShellSpec (`tests/shellspec/match-synthetic.spec.sh`); CI live-invocation 4/4 PASS — every persona flagged its planted defect against the signature predicates |
