...
- **TDD-applicable surface.** None — this slice is pure config/control-plane removal. No new logic. CODEOWNERS is declarative; hook removal is deletion. **No `tdd-exemption-allowlist.txt` entries needed (decision at impl time, session 53):** the allowlist exempts `src/**` files from the TDD-first gate (per its own L3-5 header); this slice touches no `src/**` files, so the gate doesn't fire on any of its commits. The original pre-flight wording "add slice path … at impl time" was over-prescribed — recorded here for audit-trail integrity.
...
- **Verification:**
  ...
  6. `CLAUDE.md` §"Hard controls (in development)" table: rows "Hooks-checksums drift warning" + "Control-change label requirement" removed; replacement single row "CODEOWNERS code-owner review" added with file ref `.github/CODEOWNERS` + GitHub branch-protection setting + bypass-column text reflecting solo-operator caveat: *"conscious admin-bypass click ('Merge without waiting for required review') in solo-operator context (sole code-owner = PR author; GitHub hard rule prevents self-approval); rigour gate = auto-review.yml + admin-click-as-conscious-act"*. (Original AC drafted "edit CODEOWNERS path scope under owner-review (self-protected)" as bypass text; revised at impl time per session-53 solo-operator design discovery — the conscious-admin-click is the actual gate in solo context, and the implemented text is more accurate than the original draft.)
