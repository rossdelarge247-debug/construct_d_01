...
| 10 | Pen-test posture maintained | PASS (with solo-operator caveat) | **Net: improved on attack surface; mixed on self-approval property.** Reduces attack surface (one config file vs three controls + workflow + hook script + checksums file). **Solo-operator caveat (session-53 design discovery):** in multi-developer context, CODEOWNERS + branch-protection forces approver ≠ author. In solo context (user is sole code-owner AND PR author via agent harness), GitHub's hard "PR author cannot approve own PR" rule makes the gate self-deadlock unless admin-bypass is permitted. Slice ships with admin-bypass enabled; merge of control-plane PRs requires conscious "Merge without waiting for required review" admin click. This preserves the legacy `control-change` label's "conscious act required" property but does NOT preserve the "different reviewer" property (unrecoverable in solo). The substantive review gate is `auto-review.yml` + slice-reviewer persona (machine judgement on every PR) + the user's own pre-merge review discipline. Branch-protection itself is admin-restricted; CODEOWNERS file is self-protected via the `.github/CODEOWNERS` line in CODEOWNERS. When collaborators join, the "different reviewer" property is recovered automatically by adding their `@username` line. |
