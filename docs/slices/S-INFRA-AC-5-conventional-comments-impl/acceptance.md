...
- **Control-plane change.** All three persona files are L199-protected. Hash re-baselined: `acceptance-gate.md` `889374b6…` → `03dbb343…`; `slice-reviewer.md` `10ccfc658f…` (post-PR-#37 + PR-#40 baseline on main) → `aa70bf3a…` (post-rebase; combines criterion 2 §Exceptions (a)-(e) on main + this slice's §Output format rewrite; 205L); `ux-polish-reviewer.md` `41cd40b8…` → `017fa224…`.
