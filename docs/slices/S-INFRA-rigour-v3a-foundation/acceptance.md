...
> **STATUS: SLICE PLANNING PHASE COMPLETE (session 37).** Five adversarial review iterations converged at v5 verdict `nit-only`. Trajectory: v1 (block + 6 findings) → v2 (24 findings, 2 BLOCK) → v3 (8 findings, 1 BLOCK) → v4 (4 findings, 0 BLOCK) → **v5 (2 nits, 0 BLOCK + 0 request-changes)**. v5 review at `acceptance.md.review-v5.json` recommended `proceed-to-AC-1 impl per H6 sub-sequence (S-37-1 RED commit first)`. v5.1 polish commit addresses both v5 nits as PR-comment-style follow-up: **J1** → §Bundle scope AC-8 line surfaces verdict-vocabulary circular link with AC-7 (splitting AC-8 leaves AC-7 citing undefined vocabulary — beyond the docs/system-invariant argument); **J2** → §15 table is intentionally placeholder-for-this-commit + open-ended footnote because git chicken-and-egg means a commit's own SHA cannot appear in the commit's own body (now stated explicitly here in the status banner so narrative matches table). Full review chain v1 → v2 → v3 → v4 → v5 preserved at `acceptance.md.review-v{1,2,3,4,5}.json`. **Next step:** session 38 begins with H6 sub-sequence — S-37-1 failing-meta-tests RED commit (pushed + CI-observed-failing run-ID captured) before S-37-2 verify-slice.sh skeleton.
...
- **AC-8 → all of the above (documentation-system invariant + verdict-vocabulary circular link with AC-7; J1 closure).** AC-8's CLAUDE.md "Hard controls" stub *catalogues the gates this slice ships*, AND hosts the verdict vocabulary (`approve` / `nit-only` / `request-changes` / `block` per G23) that AC-7's plan-review subagent emits and that AC-2-protected diff-review uses — splitting AC-8 leaves AC-7 citing an undefined vocabulary. So AC-8 has both a documentation-system invariant (shipping separately creates a window where the stub documents phantom gates OR the gates are undocumented) AND a real circular dependency with AC-7 via shared verdict vocabulary. AC-8 is bundled by both axes, not just discoverability.
...
- ~~Origin/main-anchored ratchet (F5c)~~ — **shipped v3c** (count-based ESLint disable ratchet + vitest coverage threshold ratchet; CODEOWNERS admin-bypass is the override path)
...
- **Self-modification protection scope (revised per G3+G10+G12+G4+F4b + H2 nonce-derivation addition; v3b AC-11 amendment adds three omissions).** AC-2 covers: `.claude/hooks/**` + `.claude/settings.json` hook-registration block + `scripts/verify-slice.sh` + `scripts/git-state-verifier.sh` + `scripts/eslint-no-disable.sh` + `.eslintrc` thresholds block + `vitest.config.ts` coverage thresholds block + `.claude/subagent-prompts/*.md` (template content with `{NONCE}` placeholder) + the nonce-derivation snippet within `.claude/hooks/exit-plan-review.sh` + `hooks-checksums.txt` + `docs/eslint-baseline-allowlist.txt` + `.github/workflows/control-change-label.yml` (workflow self-protection via path-filter self-inclusion) + `.github/workflows/pr-dod.yml`. Full origin/main-anchored ratchet for thresholds shipped v3c (per F5c). The three v3b AC-11 additions (`scripts/git-state-verifier.sh`, `scripts/eslint-no-disable.sh`, `docs/eslint-baseline-allowlist.txt`) close the silent-weakening hole surfaced at v3a session 40 — all three are integrity-baselined in `hooks-checksums.txt` but were not in the `control-change-label.yml` path filter pre-AC-11. Further candidates (`.claude/agents/`, `scripts/hooks-checksums.sh`, `docs/tdd-exemption-allowlist.txt`) deferred to v3c per AC-11 §Scope.
