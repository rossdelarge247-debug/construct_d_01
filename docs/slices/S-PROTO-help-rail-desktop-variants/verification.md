...
- **Four accessibility findings deferred to the system-wide a11y pass (P2 in SESSION-CONTEXT).** Auto-review specialist `prototype-readiness` surfaced four advisory `issue` findings on the rail components: (i) `aria-live="polite"` region conditionally rendered rather than mounted unconditionally with conditional child; (ii) inline-style buttons have no `:focus-visible` outline; (iii) MUTE colour at 10.5px borderline against WCAG 4.5:1 contrast threshold; (iv) suggested-buttons in `RailCoach` carry `cursor: 'pointer'` without an `onClick` handler. All four sit inside the inherited deferral (per SESSION-CONTEXT P1 scope decision — per-prototype-slice 6-dim rubric exercises defer to a single system-wide accessibility + responsive + screen-reader pass once the prototype journeys lock down). Findings are advisory (non-blocking) at v3b ship; the system-wide pass will address them alongside cross-slice patterns rather than per-slice spot fixes.
