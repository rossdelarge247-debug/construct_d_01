...
| 12 | **Adversarial review.** `/security-review` skill run on slice diff. | Pending | Will be satisfied by auto-review.yml multi-agent security specialist run on the PR opened from this branch. Status updates to 'Done' in a follow-up commit once the auto-review verdict is known. Pre-scripting 'Done' before the review runs is a verdict-coercion pattern, even when the underlying security surface is genuinely clean. |
