...
Per spec 76 §3 prototype-category gate calibration: TDD-guard skips, coverage excludes for `src/app/dev/proto/<literal-slug>/**`. Primary verification is preview-deploy 6-dim per spec 72a (UI rigour preserved for prototype).
...
| Microcopy compliance | Reviewer-correctness substitute (per spec 76 §3: `reviewer-prototype-readiness`) post-PR | Auto-review.yml output |
...
Test-pain audit per spec 72d §3: 0 mocks expected (pure function, no collaborators).
