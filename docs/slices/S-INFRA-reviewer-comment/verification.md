...
| AC-2 (`comment-review.sh` hook) | **PASS** | `test -x .claude/hooks/comment-review.sh` succeeds (mode `0755`). Manual smoke harness exercises stub mode (10 cases) + live mode with a mock `claude` on `PATH` (3 cases) — all 13 PASS. Hook exits 0 in every observed path; emits JSON with `systemMessage` only on findings. Live-mode logic isolated in `run_live_review` function (return-code interface) so a mock binary can drive the live path without an Anthropic API key. Framing uses `printf` concatenation so a literal `EOF` line in author content cannot terminate the envelope early. File-path normalisation strips a leading `$PWD/` so absolute paths from PostToolUse and relative paths from manual replay both match the skip-list. |
| AC-3 (`tests/shellspec/comment-review.spec.sh`) | **PASS** | `test -f tests/shellspec/comment-review.spec.sh` succeeds. `grep -cE "^[[:space:]]*It '" tests/shellspec/comment-review.spec.sh` returns `17` It-blocks across 5 nested `Describe` groups (tool-name early-exit · path skip-list · happy path · stub-mode anti-pattern detection · live mode with mock-binary `BeforeEach`/`AfterEach`). CI runs via `.github/workflows/shellspec.yml` auto-discovery. Local shellspec install absent in this sandbox; manual smoke harness (above) replays the equivalent 13-case envelope and passes 13/13 (10 stub + 3 live). |
| AC-4 (`.claude/settings.json` registration) | **PASS** | `jq '.hooks.PostToolUse[] \| select(.matcher == "Write\|Edit") \| .hooks \| length' .claude/settings.json` returns `2`. Command list returns `.claude/hooks/line-count.sh`, `.claude/hooks/comment-review.sh` in that order. `jq empty` confirms valid JSON. Outer timeout 30s (live-mode `claude -p` budget; stub mode completes <50ms in practice); inner `timeout 20 claude -p` inside `run_live_review` is the typical wait cap. |
