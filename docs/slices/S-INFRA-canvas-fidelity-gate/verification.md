...
- **Auto-review findings #1, #2, #4, #5, #6, #7 deferred at slice ship.** PR auto-review at `3318c8d` returned `request-changes` with 7 findings — 1 blocking-at-`k=1` security (path-traversal in canvas-path read; addressed inline in follow-up commit), 6 advisory:
  - **#1 (style · commenting):** WHAT-narration in `auto-review.yml` brief-compose comments. Carry-over for general comment cleanup pass.
  - **#2 (correctness · regression):** `scripts/preflight-review.sh` aggregator arg-order bug already on the carry-over backlog. Out of scope for this rigour-infra slice (preflight touches multiple subsystems and warrants its own slice).
  - **#4 (style · nitpick):** Parenthetical comment in `tests/personas/run-synthetic.sh`. Trivial; carry-over for comment cleanup pass.
  - **#5 (correctness · ac-gap):** §AC-3 verification claim "auto-review pipeline running on this PR exercises every script path" overstated for `infrastructure`-category PR — canvas-fidelity-specific routing isn't exercised here; first real exercise is the rebuild slice's prototype PR. Carry-over for AC-3 evidence requalification at rebuild-slice ship.
  - **#6 + #7 (correctness · spec-citation):** Reframe-to-bare-spec fix is syntactic compliance with `spec-citation-quote-check` rather than substantive verbatim-quote per CLAUDE.md §"Quote, don't paraphrase". Discipline distinction: navigational doc-pointers (`spec 76 §3` after a category declaration) are not load-bearing claims of spec authority; "per spec X" form is the trigger because it asserts authority. Comprehensive verbatim-quote audit across slice docs is a separate convention-shift slice (~50L of edits), not in scope for this rigour-gate slice.
