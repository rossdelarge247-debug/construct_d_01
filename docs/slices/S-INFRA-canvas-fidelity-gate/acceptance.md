...
- **Verification:**
  ...
  2. File size ≤300L spec 72c §4 directive (target ≤200L via include-by-reference for verdict vocab + JSON schema).
  ...
  4. File specifies per-invocation context fences: `<pr-diff-NONCE>` · `<slice-ac-NONCE>` · `<linked-canvas-NONCE>` (the new fence — content is the linked canvas .jsx or .html file inlined spec 72b Option C nonced delimiters) · `<verdict-vocab-NONCE>` · `<spec-72c-section-5-NONCE>`.
  5. File specifies strict-JSON output envelope spec 72c §5 with `specialist: "reviewer-canvas-fidelity"` + `findings[]` shape matching existing personas (label · blocking · category · evidence · remediation).
