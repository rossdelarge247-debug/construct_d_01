...
- **Outcome:** `scripts/criterion-2-exception-check.sh` reads a list of changed files (one per line) on stdin and emits one tab-separated line per file: `<path>\t<exception-id|none|requires-judgement>\t<reason>`. Hardcoded path globs cover ids `c` (`docs/workspace-spec/**` + `docs/design-source/**`) and `e` (`docs/HANDOFF-SESSION-*.md` + `docs/SESSION-CONTEXT.md`); `docs/slices/<id>/{acceptance,verification,security}.md` pass through as `requires-judgement` (exception (b) candidate); other paths emit `none`. The script does not parse the YAML at runtime — globs are kept aligned with `predicate.paths_in` arrays in the YAML by the head-comment convention. `tests/shellspec/criterion-2-exception-check.spec.sh` exercises 14 cases including each exception's positive path, anchored-glob non-match (`docs/handoffs-archive/HANDOFF-SESSION-12.md` → `none`), multi-file order preservation, blank-line skipping, empty-stdin → empty-output, and space-bearing paths.
- **Verification:**
