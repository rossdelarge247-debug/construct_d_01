...
- **Outcome:** Component at `src/components/document-shell/DocumentShell.tsx`. Renders a top-bar slot above a CSS-grid three-column body. Desktop column template: left rail ~240px · body 1fr · right rail ~280px. Top bar slot per spec 68d S-D2 + S-D4: `title: string`, `state: DocumentState` (`'draft' | 'ready-to-send' | 'counter-received' | 'in-progress' | 'agreed'`), `autosaveStamp?: string`. Body slot is required; left rail / right rail slots optional (a document can render without them — Settlement Progress board may not need a TOC). Slot interface uses flat React props (`header`, `leftRail`, `body`, `rightRail`) typed `ReactNode`. No content opinion at this layer — slot consumers own content shape. Polymorphic body element via `bodyAs?: 'main' | 'section'` prop (default `'main'`); host page that already owns a `<main>` landmark passes `bodyAs="section"` to avoid nested mains.
...
- **Verification:** CSS-driven responsive (toggles always in DOM, hidden at desktop via Tailwind class). Component test renders the shell once and asserts the DOM contract: both toggle buttons present with the correct responsive Tailwind class — left toggle carries `lg:hidden` (CSS-hides at desktop ≥1024px); right toggle carries `md:hidden` (CSS-hides at tablet/desktop ≥768px). Toggle buttons carry `aria-expanded` reflecting React state + `aria-controls` referencing the rail's `id`. Rail open/close state stored in component-local React state; `data-state="open" | "closed"` attribute on rails for CSS targeting. Preview-deploy at three viewport widths (375 · 800 · 1280) verifies CSS hides toggles + rails at the right widths.
...
- **Outcome:** Shell is fully keyboard-navigable + screen-reader-friendly + respects motion preference:
  ...
  - **Region landmarks** — top bar wrapped in `<header>` (banner role implicit); left rail wrapped in `<nav aria-label="Document sections">` when present; body wrapped in `<main>` by default OR `<section>` when host passes `bodyAs="section"` (per AC-1 polymorphic body); right rail wrapped in `<aside aria-label="Document context">` when present. One `<main>` per page is the host's responsibility — the shell defaults to `<main>` for the common case where the shell is the page's primary content; demo pages that wrap multiple non-shell content above the shell pass `bodyAs="section"` so the host's outer `<main>` stays the page's sole main landmark.
  ...
  - **Focus management on toggle** — when a toggle button is clicked and the rail opens, focus moves to the first focusable child of the rail (anchor / button / `[tabindex]:not([tabindex="-1"])` / form control). When the user presses `Escape` while focus is inside the rail, the rail closes and focus returns to the toggle button. Pure component-local React state + `useEffect` watching open-state + `useRef` on rail and toggle elements; no global focus-trap library.
...
- **Outcome:** Existing placeholder landing page (the S-F1/S-F3/S-F4 demo grid) extended with a "Document Shell" demo block. Page outer wrapper stays `<main>` (the host page's primary landmark for all the demo blocks). The block renders `<PhaseStepper currentPhase="build" phases={phases} />` above a `<DocumentShell bodyAs="section" ...>` populated with Sarah's-Picture-shaped stub content (the `bodyAs="section"` override avoids a nested-main since the page's outer wrapper already owns the `<main>` landmark):
...
| 2026-05-04 | Auto-review (architecture + correctness specialists) + User | **Round 2 amendments** | Architecture caught the `<main>` → `<div>` page-wrapper scope creep (first formal retain catch for reviewer-architecture). Resolved by adding `bodyAs?: 'main' \| 'section'` prop to AC-1 (default `'main'`); demo passes `'section'`; page outer wrapper restored to `<main>`. AC-3 amended to record `bodyAs` polymorphism + focus management on toggle (focus first focusable child on open, Escape closes + returns focus to toggle). AC-4 amended to declare `bodyAs="section"` usage in the demo + restored page wrapper. Module-level `BODY_ID` replaced with `useId()`-derived `bodyId` (correctness catch on multi-shell collisions). |
