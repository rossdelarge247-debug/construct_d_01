...
| 5 | No regression in adjacent slices | **PASS** | All 12 pre-existing shellspec It-blocks for `auto-review-parse.sh` continue to assert the same `output should equal` values without modification — schema validation is bolted on AFTER stdout emission so it cannot affect existing test expectations. The validator itself (`scripts/validate-finding-envelope.sh`) and its 16 spec cases are untouched |
