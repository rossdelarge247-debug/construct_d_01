...
| 5 | Input validation at system boundaries | PASS | Script defensively guards against malformed input: empty stdin, non-object JSON root (array / string / number / null), non-JSON garbage, empty `{}` sentinel. All map to `parse-failed` rather than crashing with a confusing jq error. The 16-case shellspec test contract includes 5 explicit malformed-input cases (test cases 9-14). |
...
| 10 | Pen-test surface change | NEUTRAL | New attack surface: "what can an attacker do by getting the slice-reviewer persona to emit specific findings JSON?" Threat model: (a) prompt-injection in PR diff content → persona emits attacker-controlled findings JSON → script derives verdict from findings shape only (not textual content). **Mitigation tested:** test case 16 (verdict-coercion guard per spec 72c §5 rule 3) verifies that prompt-injection in finding `evidence` / `summary` text does NOT influence derivation. (b) Adversarial output shape (e.g. `{label: "issue", blocking: "true"}` string-vs-bool) → jq's `select(.blocking == true)` is strict-equal; string `"true"` doesn't match. ACTION_COUNT increments instead → request-changes (defensible). Tested in case 8. |
...
2. **16-case test contract** — covers each of the 4 verdict outputs (block / request-changes / nit-only / approve) at least once, plus the parse-failed sentinel and 5 adversarial / malformed-input cases.
3. **Local + CI verification** — `shellspec` reports `16 examples, 0 failures` (this spec) + `104 examples, 0 failures` (full suite, no regression in adjacent tests).
