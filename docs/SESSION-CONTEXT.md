...
Deployment: Vercel — preview deployments per branch, production at `construct-dev.vercel.app`. During Phase C, production stays **frozen** on V1; rebuild lives on `phase-c` long-lived integration branch with per-slice Preview URLs (spec 71 §7a).
...
6. **Do NOT** redeploy production during Phase C. Prod (`construct-dev.vercel.app`) stays **frozen** on V1 per spec 71 §7a. All new work lands on `phase-c` integration branch; cutover is a single atomic fast-forward at launch readiness. Hotfix protocol = cherry-pick same-day into `phase-c`.
...
Post-merge of PR #3 (session 23), `main` carries sessions 19-23's combined output as canonical.

**Session 24 branch plan (per spec 71 §7a Phase-C-freeze topology):**
- Current (session-ops / SESSION-CONTEXT rewrite): `claude/update-session-context-ZrUCG` (this rewrite commits here).
- Integration branch: **create `phase-c`** long-lived from `main` as Track A's first ops step.
- Per-slice branches: `phase-c/S-XX-{scope}` off `phase-c`. Never push direct to `main` during Phase C.
- Preview URLs: prod (`construct-dev.vercel.app`) stays frozen; `construct-dev-git-phase-c-*.vercel.app` rolls with integration branch; `construct-dev-git-<slice-branch>-*.vercel.app` per slice.
- Cutover at Phase C launch readiness: fast-forward `main` ← `phase-c` in a single atomic prod deploy.

**Pre-flight verification before Track A starts:**
```
git fetch origin main
git checkout main
git pull --ff-only origin main
```
Confirm `main` tip matches the merged PR #3 commit before branching `phase-c`.
