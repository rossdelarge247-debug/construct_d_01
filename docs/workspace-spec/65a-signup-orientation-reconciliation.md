...
Session 126 (S-PROTO-sign-in): decision B extends decision A to sign-in. The sign-in artboard (`#m-signin`, mobile-screens-v2 "05a · Mobile · Sign in") carries *"Continue with Google"* and *"Sign in with passkey"* beneath the password form; the build renders email · password · *"Forgot?"* · remember-device · *"Sign in"* only, so the two auth surfaces agree. Google OAuth and passkeys stay logged for user testing alongside magic-link, not decided here. Password reset remains blocked with 1.2a (no canvas).
