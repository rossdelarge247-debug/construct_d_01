...
**Status:** Amended session-70 (§3 architecture diagram · §4 specialist partition · §5 aggregation math reduced from 4 to 3 specialists — `reviewer-architecture` dropped per CLAUDE.md §"Persona retain/drop metric" formal drop trigger; cumulative findings well below the 0.33 retain bar across the 13-PR cohort through session 69). Prior amendments: session-56 (§5 default flip `k=1` → `k=2` · §5 shadow field rename · §5 §Revisit triggers reframed post-flip · §10 lineage entry updated); session-54 (§3 architecture diagram · §4 specialist partition realigned to /ultrareview 4-partition · §5 aggregation flipped to majority-vote default with shadow-monitor · §7 fixture harness flipped to golden-PR replay primary · §9 + §10 markers updated); session-49 (§5 revisit trigger · §7 hybrid extension · §10 pattern lineage). Original: Draft (v3b S-8 stretch); standalone spec PR opened before `S-INFRA-persona-suite-v2-multi-agent` slice acceptance.md was frozen, per session-48 pre-flight Q3 (option B, "Now between v3b and S-F1").
...
The session-70 reduction to 3 (via architecture drop) is distinct from the originally-discussed §9 carry-over compression option (folding `style` into `correctness`); the former was data-driven (cumulative catch-rate measurement triggered the drop), the latter is structural (which rubric dimensions cluster). Architectural-smell-trigger watch (CLAUDE.md §"Engineering conventions" §"Architectural-smell trigger") moves to manual-judgement at multi-round patch-iteration; not a per-PR specialist after session-70. Expansion to 4 (re-introducing a separate `regression` or `spec-citation` specialist) is still tracked at §9.
...
- **Pass criterion:** aggregator verdict on the replayed diff matches `prior-verdict.json` exactly at the verdict tier (`approve` / `nit-only` / `request-changes` / `block`); finding count within ±1 of `prior-findings.json` length; per-specialist `seen_by[]` overlap ≥50% with the prior recorded findings (i.e. the same dimension flagged the same issue, not a different specialist coincidentally flagging an unrelated finding). Note: PR #30 seed was captured pre session-70 with 4 specialists including reviewer-architecture; replay against the post session-70 3-specialist suite expects the architecture-dimension `seen_by[]` entries to register as drift but NOT verdict change (the session-70 drop was data-driven on cumulative low catch-rate).
...
| Findings-per-round | ~1.6 | ≥3 (one per active specialist post session-70) | retain if specialists fire independently |
...
**Session-70 drop verdict (architecture specialist):** cumulative catch-rate well below the 0.33 retain bar across the 13-PR cohort through session 69 (auto-review fired, architecture specialist returned 0 substantive catches across the streak). Drop verdict landed in PR #112 (session 70) — `.claude/agents/reviewer-architecture.md` removed; `auto-review.yml` matrix + `tests/personas/run-{synthetic,replay}.sh` + `scripts/spawn-multi-reviewer.sh` + `scripts/derive-verdict.sh` all reduced to 3-specialist iteration. Validates that the retain/drop measurement framework actually triggers a drop when data says drop — the metric is meaningless if nothing ever gets dropped.

## §9 — Open questions / v3c carry-overs

- **Specialist count tuning (3 vs 4 vs 5).** Live state post session-70 is 3 (security · correctness · style) following the architecture drop verdict. Pre session-70 was 4 per /ultrareview convergence (§10 L194). Expansion to 4 (re-introducing a separate `regression` or `spec-citation` specialist; or adding a UI specialist parallel to the 3 once `ux-polish-reviewer.md` activates at S-F1) tracked here. Architectural-smell-trigger watch (CLAUDE.md §"Engineering conventions") moves to manual-judgement post session-70; can be promoted back to specialist if structural patterns warrant. **See also spec 72d** — B+C+D pre-impl rigour additions (D test-pain gate · B fitness functions · C plan-architect persona) contract an adjacent programme covering pre-impl architectural review at three lifecycle stages, complementing the post-drop landscape without restoring the dropped specialist.
- **`k`-quorum post-flip calibration.** Default flipped from `k=1` to `k=2` (now majority across 3 specialists) at session 56 on the strength of n=2 infra-PR shadow data — see §5 §Revisit triggers for the post-flip false-negative trigger (flip back to `k=1` if first-3-src-slice false-negative rate >20%) and the orthogonal severity-weighted-aggregation upgrade path (per *Beyond Majority Voting* §3.2). Aggregator emits shadow `would_have_been_k1` / `would_have_been_k3` for every PR review; `would_have_been_k1` is the primary post-flip calibration signal. `k=3` shadow now represents unanimity rather than supermajority.
- **Synthetic-deliberate-injection per-persona fixtures.** Deferred to v3c per §7 (golden-PR replay primary at v3b ship). Add once first-3-src-slice retain/drop data confirms the 3-partition holds; synthetic catches per-persona regressions that golden-replay can't isolate. Path: `tests/personas/synthetic/{security,correctness,style}.diff`.
...
- **§4 specialist partition** — `/ultrareview` for Claude Code (4-specialist published shape: security · architecture · correctness · style). 72c shipped at 4 (session-54 realigned per this prior-art finding; original draft was 7 — the four post-session-47 attention-vector dimensions plus three speculative ones); 72c diverged from `/ultrareview` to 3 specialists at session-70 (architecture dropped per CLAUDE.md retain/drop metric — data-driven divergence from the published 4-shape). Compression to 3 + expansion to 5 still tracked at §9 as carry-overs (compression now achieved; expansion remains open).
- **§5 quorum-based aggregation** — aligned with mainstream majority-vote / reliability-weighted (*Beyond Majority Voting*, arXiv 2510.01499; *LLM Jury-on-Demand*, arXiv 2512.01786) via configurable `k`-of-N quorum. Default `k=2` post session-56 amendment (was quorum-of-half across 4 pre session-70; now majority across 3). Shadow `k=1`/`k=3` emitted for monitoring; `k=3` now unanimity at 3 specialists. Revisit triggers track post-flip false-negative rate per §5.
...
6. https://www.claudedirectory.org/blog/ultrareview-claude-code-guide — Anthropic-adjacent published 4-specialist persona partition (72c diverged to 3 at session-70).
