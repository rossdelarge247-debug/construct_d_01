...
- **Multi-provider 3rd-agent reviewer** — spec 71/72 v3c carry-over from `docs/slices/S-INFRA-rigour-v3a-foundation/acceptance.md` L177. A non-Anthropic specialist (OpenAI / local model) as a tie-breaker when two specialists disagree on architectural severity. Out of scope for this spec; surfaces here for cross-reference.
