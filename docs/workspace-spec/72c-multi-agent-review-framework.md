...
- **Multi-provider consensus framework** — `docs/slices/S-INFRA-rigour-v3c-quality-and-rewrite/acceptance.md` §"Multi-provider consensus framework (candidate; session-48 addition)". Original v3a scope was a non-Anthropic specialist as tie-breaker on architectural-severity disagreement (acceptance.md L177); session-48 extension promotes this to N-provider parallel review with consensus required for merge — same cognitive-diversity argument as dimension-partition (§1) but on the provider axis instead of the rubric axis. Three open questions parked at v3c kickoff: consensus rule, provider mix, cost/signal trade-off. Decision gate: first 3 src/ slices under 7-dim Claude-only baseline supply the measurement data for the architecture choice. Out of scope for this spec; surfaces here for cross-reference.
