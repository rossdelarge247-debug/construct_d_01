...
## 5. Discard-tree removal

**Amendment — session 24, Option 4:** Bulk-removal applied upfront at Phase C kickoff. The original "per-slice staged removal" model below was designed around a "preview deploys never fully broken" constraint inherited from the Phase-C-freeze topology. With that topology retired (see §7a amendment below), and with no V1 users to protect, staged removal paid bookkeeping cost without buying anything. The Discarded tree was deleted in a single Phase C foundation PR; `src/app/page.tsx` was replaced with a placeholder landing; `src/types/interview.ts` and `src/components/interview/*` and `src/components/workspace/*` and `src/app/{features,pricing,start,workspace}/*` (except `src/app/workspace/engine-workbench` which was Re-use) removed wholesale.

Slices still update spec 70 hub inventory when they touch adjacent preserved code, but don't carry Discarded-deletion responsibility. The removal sequence table below is now **historical** — most deletions happened in the Option 4 foundation PR. Retained for reference showing which slices originally owned which surfaces.

**Historical removal sequence table** (pre-Option-4 plan):
...
```
Phase C foundation PR merged to main (session 24, Option 4) — V1 wiped, new scaffolding in place
  → C.1 foundation slices on main (S-F1 → S-F7 → S-F3 → S-F2 → S-F4 → S-F6)
  ...
  → first-real-user-ready (no cutover event — main has been the rebuild home throughout)
```
...
## §7a · Deployment model — superseded by Option 4 (session 24)

**Amendment:** The original Phase-C-freeze topology below — long-lived `phase-c` integration branch, frozen main, atomic cutover — is **retired**. Reason: the freeze model was designed around protecting real users on V1 during rebuild. Decouple has no users. Every rationale the original text gave for freeze evaporates when there's nothing to protect:

| Original spec rationale | Status under Option 4 |
|---|---|
| Real users on current prod uninterrupted | Moot — no users |
| No patchwork user experience mid-rebuild | Moot — no users |
| Clean atomic cutover with single rollback path | Moot — no V1 to roll back to |
| Integration Preview reflects "what prod will be at cutover" | Moot — no cutover event |
| Per-slice Previews don't affect prod | Moot — prod has no protected value |

### Replacement model: single-branch-main

- `main` is the single canonical branch through Phase C.
- Slice work happens on short-lived feature branches (`claude/S-XX-{slug}` or similar) merged via PR to main.
- Vercel prod URL (`construct-dev.vercel.app`) rebuilds on every main merge; may show a placeholder / partially-built state during rebuild — acceptable given no users.
- No cutover event. No integration branch.

### What's preserved from the original §7a

- Per-slice Preview URLs still work (Vercel creates them for every PR branch).
- Per-slice DoD discipline unchanged (spec 72 §11 security checklist still required).
- Env scoping still applies: Vercel Production `DECOUPLE_AUTH_MODE=prod`, Preview `=dev`, local `=dev`.
- Hotfix protocol moot (nothing to hotfix).

### If user traffic arrives before Phase C completes

Unexpected but possible. In that case: re-introduce a freeze + cutover via a new amendment, don't retrofit from this spec's pre-Option-4 text. The freeze model's details depend on the shape of the user traffic (beta cohort vs self-serve signups), which would inform the topology choice fresh.

---

### Original §7a (retained as historical reference — **superseded**)
...
~~**Three testable surfaces during Phase C:** prod frozen · `phase-c` integration Preview · per-slice Preview.~~

~~**Branch flow:** `phase-c` from main at freeze-start → slice branches off `phase-c` → merge slice to `phase-c` → cutover = fast-forward `main ← phase-c`. Main stays frozen. Hotfixes cherry-picked same-day.~~

~~**Cutover checklist:** DPIA · pen-test · legal review · real Supabase with RLS · beta users via integration Preview · go/no-go documented.~~
