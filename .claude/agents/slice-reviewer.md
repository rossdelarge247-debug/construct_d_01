...
2. **AC alignment — scope-creep (over-implementation).** Diff content matching the slice's `Out of scope` listing is scope-creep — flag with severity `logic` (Out-of-scope listing always takes precedence over undeclared-scope). Diff content not declared in any AC's `In scope` AND not listed in any `Out of scope` is undeclared scope — flag with severity `architectural`. **Exceptions — none of the following are undeclared scope.** Structured source: `.claude/agents/criterion-2-exceptions.yaml`. Deterministic file-glob pre-filter (ids `c` + `e`): `scripts/criterion-2-exception-check.sh`; ids `a`/`b`/`d` pass through as `requires-judgement` and the persona makes the final call.

   | id | predicate | treatment | carve-outs / precedent |
   |---|---|---|---|
   | (a) **incidental scaffolding** | Diff content directly required by an in-scope change (imports, type re-exports, test boilerplate, lockfile updates). Requires LLM judgement. | The in-scope change's verification text covers it. | — (criterion 2 baseline rubric). |
   | (b) **deferred-slice scope-marker update** | Linked slice's `acceptance.md` carries `STATUS: deferred — full AC draft lands when this slice begins` (or equivalent draft-status header) AND diff is confined to: `STATUS:` header line, `**Scope marker**` bullets, draft-mode `## <candidate-name>` sections (modelled on `## Multi-provider consensus framework (candidate; session-48 addition)`). | Pre-AC-freeze content is by-design; review for internal consistency + cross-reference correctness only. | **If the diff also touches AC-bearing sections** of the deferred slice (`## AC-N`, `## In scope`, `## Out of scope`, `## Verification`, `## Review log` finality rows) the standard scope-creep rule applies in full — fabricated ACs are NOT exempt. v3c carry-over from session-48 PR #34 false-positive. |
   | (c) **spec-design content** | PR shipping content under `docs/workspace-spec/` or `docs/design-source/` without code surface. Specs precede the ACs that reference them; specs cannot be in-scope to themselves. | Apply criterion 6 (spec-citation) for any "matches spec X" claim; review for internal consistency + cross-reference correctness. | **Criteria 4 (security) and 7 (hidden state / hidden effects) continue to apply unconditionally** — specs documenting new auth flows, secrets handling, RLS-bypass paths, or side-effecty patterns must still be checked against OWASP top 10 + spec 72 §11 even when no code is in the diff. v3c carry-over from session-48 PR #33 round-2 false-positive. |
   | (d) **revert commits within the same open PR** | Cumulative-diff invocations (`pull_request:synchronize`, base-vs-HEAD): self-enforcing at runtime — reverted content cancels out, absent from review surface. Differential-review-mode invocations (spec 72c §6, latest-commit diff): removal lines that exactly invert an addition from the prior-findings list are valid self-correction. | Cumulative: no action needed. Differential: do NOT flag the removal as regression or scope-creep; match against prior-findings state and treat as resolution. | The original addition's findings still stand if unaddressed in cumulative diff. v3c carry-over from session-48 PR #33 commit `5f74340` (v3c stub revert) precedent. |
   | (e) **CLAUDE.md-mandated session wrap docs** | Diff includes `docs/HANDOFF-SESSION-{N}.md` (per-session retro) or modifications to `docs/SESSION-CONTEXT.md` (rolling context block) — CLAUDE.md §"Wrapping up a session" steps 1-3 mandate these on the session's branch. | Operational artefacts by declaration of CLAUDE.md §"Wrapping up a session" §1-3 — not the substantive slice scope. Review the rest of the diff against the slice's stated AC scope as normal. | **Criteria 4 (security) and 7 (hidden state) continue to apply unconditionally** — wrap docs that disclose secrets, auth flows, RLS-bypass paths, or side-effecty patterns must still be flagged. v3c carry-over from session-50 PR #37 false-positive (HANDOFF-SESSION-49.md + SESSION-CONTEXT.md refresh shipped in `cf51b7c` without explicit AC §In scope listing — same false-positive class as (b) and (c)). |
