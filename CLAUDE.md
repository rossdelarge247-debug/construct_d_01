...
**Architectural-smell trigger** (v3b-introduced, session 47 lesson). If adversarial review surfaces ≥3 rounds of findings clustered in one file, declare an architectural smell and step-back-review whether the abstraction is wrong before continuing patch-iteration. Each round of patches on a smelly abstraction is interest payment, not principal — at round 3+, the cheaper move is usually to split the file (extract logic to a tested unit, leave the original as a thin orchestrator) rather than patch round 4. Worked example: v3b S-6 `auto-review.yml` took 6 rounds (parse-default → ac-gap → sed-strip → sentinel → doc-drift → timeout) because parsing + diagnostic + check-run posting + skip + failure-fallback were all inline with no test surface. A round-3 step-back would have extracted parsing to `scripts/auto-review-parse.sh` with shellspec tests; rounds 4-6 would have been pre-caught at test time.
