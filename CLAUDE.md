...
- **`acceptance-gate.md` (AC-2)** — spawned at slice-completion time by `/wrap` or main-session call. Inputs: slice `acceptance.md` + slice `verification.md` + CLAUDE.md "Engineering conventions §Definition of Done". Verdict is informational at v3b ship; auto-blocking PR merge deferred to v3c per AC-2 §Out of scope.
...
All three personas emit strict-JSON output per the `Verdict vocabulary` above. Persona prompts include nonced fenced delimiters for each input, and accept spec 72b Option C inline-file content (`--- BEGIN <path> --- ... --- END <path> ---`) for atomic files >300 lines.
